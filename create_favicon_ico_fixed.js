...
function createIcoFile() {
  try {
    ...
    for (const favicon of faviconSizes) {
      ...
      console.log(`✓ Loaded: ${favicon.file} (${buffer.length} bytes)`);
    }
    ...
    for (let i = 0; i < faviconSizes.length; i++) {
      ...
      // Use PNG data size as image data size
      ...
      const dirEntryBuffer = dirEntry.toBuffer();
      dirEntryBuffer.copy(icoBuffer, pos);
