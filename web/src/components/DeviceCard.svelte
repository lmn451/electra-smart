...
<div class="card" data-id={id}>
  <div class="header">
    ...
    <div class="badge {fields?.isOn ? 'on' : 'off'}">
      {fields ? (fields.isOn ? "ON" : "OFF") : "—"}
    </div>
  </div>
  <div class="grid">
    <div class="kv">
      <span class="k">Mode</span><span id={`mode-${id}`}
        >{fields?.mode ?? "—"}</span
      >
    </div>
    <div class="kv">
      <span class="k">Fan</span><span id={`fan-${id}`}
        >{fields?.fan ?? "—"}</span
      >
    </div>
    <div class="kv">
      <span class="k">Setpoint</span><span id={`spt-${id}`}
        >{fields?.spt ?? "—"}</span
      >
    </div>
    <div class="kv">
      <span class="k">Current</span><span id={`cur-${id}`}
        >{fields?.current ?? "—"}</span
      >
    </div>
  </div>
  <div class="controls">
    ...
    <div class="tempctl">
      ...
      <input
        ...
        step="1"
        placeholder="Temp °C"
        aria-label="Setpoint temperature in Celsius"
      />
      <button
        class="btn"
        on:click={() => {
  ...
  <div class="actions">
    <button class="btn" on:click={() => power(true)} disabled={working}
      >Power On</button
    >
    <button class="btn" on:click={() => power(false)} disabled={working}
      >Power Off</button
    >
  </div>
  {#if editing}
